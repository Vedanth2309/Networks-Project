...
double EHO::CalculateFitness(
    ...
    for (uint32_t i = 0; i < demand.size(); ++i) {
        ...
        squaredAllocation += allocation[i] * allocation[i];

        if (allocation[i] < demand[i]) {
            unmetDemand += demand[i] - allocation[i];
        }
    }
    ...
    if (totalDemand > 0.0) {
        unmetDemandRatio = unmetDemand / totalDemand;
    }

    double fairness = 1.0;

    if (squaredAllocation > 0.0) {
        fairness = (totalAllocation * totalAllocation) / (static_cast<double>(demand.size()) * squaredAllocation);
    }

    double capacityPenalty = 0.0;

    if (capacity > 0.0 && totalAllocation > capacity) {
        capacityPenalty = (totalAllocation - capacity) / capacity;
    }

    double utilizationPenalty = 0.0;

    if (totalDemand > capacity && capacity > 0.0) {
        double utilization = totalAllocation / capacity;

        utilizationPenalty = std::max(0.0, 1.0 - utilization);
    }
...
std::vector<double> EHO::GenerateRandomAllocation(
    const std::vector<double>& demand,
    double capacity) {
    std::vector<double> allocation(
        demand.size(),
    ...
    for (uint32_t i = 0; i < demand.size(); ++i) {
        allocation[i] = demand[i] * random01(m_generator);
    }
...
std::vector<double> EHO::Optimize(
    const std::vector<double>& demand,
    double capacity) {
    if (demand.empty()) {
        m_bestFitness = 0.0;
    ...
    for (uint32_t iteration = 0; iteration < m_iterations; ++iteration) {
        ...
        uint32_t clanCount = std::max<uint32_t>(1, m_numberOfClans);

        uint32_t clanSize = population.size() / clanCount;

        if (clanSize == 0) {
            clanSize = 1;
        }

        for (uint32_t clan = 0; clan < clanCount; ++clan) {
            uint32_t start =
                clan * clanSize;
            ...
            std::vector<double> leader = population[bestIndex];
            ...
            for (uint32_t i = start; i < end; ++i) {
                ...
                for (uint32_t j = 0; j < demand.size(); ++j) {
                    double r =
    ...
    m_bestFitness = globalBestFitness;

    return globalBest;
}

double EHO::GetBestFitness() const {
